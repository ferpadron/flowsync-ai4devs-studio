...
## Reglas de proceso
- La rama es por unidad de trabajo, no por petición: si ya estás en una rama de trabajo que no sea `main` ni una `sN/*`, sigue en ella en vez de crear otra. Solo al empezar una unidad de trabajo nueva desde `main`/`sN/*`: crear una rama (`git checkout -b feat/<slug>`). Nunca commitear directo en `main`/`sN/*`.
- El commit es por petición: al terminar cada petición, usar la skill `/commit`.
- Cuando un cambio toque rutas, controladores, validators o transformers de una capability, revisar antes del commit que su contrato OpenAPI y su README modular sigan reflejando el código. Si cambia el contrato HTTP, actualizar en ese mismo commit los decoradores o esquemas OpenAPI correspondientes (el documento se genera en runtime a partir de ellos; no hay un archivo estático que regenerar). Si cambia el mapa operativo, el alcance, los endpoints, las reglas resumidas, los comandos o la ubicación del código, actualizar en ese mismo commit `docs/capabilities/<nombre>/README.md`. No modificar ninguno de los dos artefactos si el cambio no afecta su contenido.
- `gh pr create` y el pase del subagente `adversarial-reviewer` van una sola vez, al terminar la unidad de trabajo entera, no al cerrar cada petición: abrir el PR con una descripción completa de los cambios en el cuerpo, y después pasar `adversarial-reviewer` sobre él antes de darlo por terminado.
